    ...
    class VSTS < RequestSource
      ...
      def update_pull_request!(warnings: [], errors: [], messages: [], markdowns: [], danger_id: "danger", new_comment: false)
        ...
        if new_comment 
          ...
          update_old_comment(danger_id: danger_id)
        end
      end

      def update_old_comment(danger_id: "danger")
        @api.fetch_last_comments.each do |c|
          ...
          @api.update_comment(thread_id, comment_id) if comment_content.include?("generated_by_#{danger_id}")
        end
      end
    end
  end
